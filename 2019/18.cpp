...
//    static bool keys[26];
    ...
    std::vector<int> route[7] = {
         std::vector<int>({1,'f','q','w','r'}),
         std::vector<int>({0,'a','c','h'}),
         std::vector<int>({2,'v','o'}),
         std::vector<int>({1,'j','m'}),
         std::vector<int>({3,'u','k','l','t','z','g'}),
         std::vector<int>({0,'b'}),
         std::vector<int>({2,'e','s','d','i','n','p','x','y'})
    };

    int walker_x = x_a,walker_y=y_a,total_steps = 0, key_cnt=0;
    for (int i=0;i<26;i++)keys[i]=false;

    for (int route_step=0;route_step<7;route_step++) {
        int dx,dy;

...
int main() {
    ...
    for (int i=0;i<1000;i++)for(int j=0;j<1000;j++)lights[i][j]=false;
    int total_lights = 0;
    char a,dummy;
    int command;
    int x1,x2,y1,y2;
...
{std::cerr<<"OEI!"<<std::endl;return 1;}
        ...
        for (int i=x1;i<=x2;i++)for(int j=y1;j<=y2;j++)switch(command){case 2:lights[i][j]=!lights[i][j];break;case 1:lights[i][j]=true;break;case 0:lights[i][j]=false;break;default: std::cerr<<"IK SNAP "<<command<<" NIET"<<std::endl;return 1;}
    }
    for (int i=0;i<1000;i++)for(int j=0;j<1000;j++){std::cout<<(lights[i][j]?1:0)<<' ';total_lights+=lights[i][j]?1:0;}
